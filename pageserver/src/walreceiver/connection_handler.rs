...
use utils::{lsn::Lsn, pq_proto::ZenithFeedback, zid::ZTenantTimelineId};
    ...
    pub fn new(
        ...
        let handle = tokio::spawn(
            async move {
                ...
                .await
                .map_err(|e| {
                    format!("Walreceiver connection for id {timeline_id} failed with error: {e:#}")
                });

                match &connection_result {
                    Ok(()) => {
                        debug!("Walreceiver connection for id {timeline_id} ended successfully")
                    }
                    Err(e) => warn!("{e}"),
                }
                events_sender
